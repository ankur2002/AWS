...
def ec2_create_copy_image(ec2,ec2_dest_client,id,source_instance_dict,source_volume_info_dict, source_snapshot_dict):
	"""
...
def ec2_dest_copy(ec2_dest, ami_dict):
	...
	for name,id in ami_dict.items():
		...
		dest_image = ec2_dest(dest_image_id)

		dest_image.wait_until_exists()
		print("This has been copied to the destination:", dest_image_id)

		if name not in ami_dest_dict.keys():
			ami_dest_dict[name] = dest_image_id

	return ami_dest_dict
...
def create_instance(source_instance_info,ami_dict):
	for instance_name in ami_dict.keys():
		source_security_group_name = []
		destination_security_group_id = []
		source_instance_id = instance_grep(instance_name,'source','name')
		source_subnet_id = source_instance_info[instance_id]['subnet_id']
		source_security_groups = source_instance_info[instance_id]['security_groups']
		source_subnet_name = subnet_grep(source_subnet_id,'source','id')
		ami_id = ami_dict[instance_name]
		tags = source_instance_info[instance_id]['tags']
		...
		ec2_dest.create_instances(MaxCount=1,MinCount=1,SecurityGroupIds=destination_security_group_id,SubnetId=dest_subnet_id,TagSpecifications=tags,ImageId=ami_id)
...
def create_route_tables(source_route_tables, ec2_dest, vpc_id, count=0):
		route_tables = []
		x = 0
		#vpc_id = info['VpcId']
		while x <= count:
...
if __name__ == '__main__':
	...
	for id in source_instance_dict.keys():
		print(id)
		instance_name, image_id = ec2_create_copy_image(ec2, ec2_dest_client, id, source_instance_dict, source_volume_info_dict, source_snapshot_dict)
		if instance_name not in ami_dict.keys():
			ami_dict[instance_name] = image_id

	ami_dest_dict = ec2_dest_copy(ec2_dest, ami_dict)
	print(ami_dest_dict)
...
#Skipped over peering for now since this is bound to only copy resources between regions
		...
		if not check_vpc_ignore(source_vpc):
			...
			for igw in source_igw['InternetGateways']:
				...
				if not check_igw(source_tags,dest_vpc_id):
					create_igw(source_tags,dest_vpc_id)
				else:
	...
	for vpc_id, vpc_params in vpcs_dest.items():
						...
						if source_rt_count > dest_rt_count:
							#dest_route_tables = create_route_tables(source_route_tables, ec2_dest_client, vpc_id, count=source_rt_count)
						...
						if len(dest_route_tables) >= 3:
							...
							for ig in dest_igw['InternetGateways']:
						...
						for id in source_instance_dict.keys():
							dest_image = ec2_create_copy_image(ec2,ec2_dest,id,source_instance_dict)
							print(dest_image)

						create_instance(source_instance_dict,ami_dict)
